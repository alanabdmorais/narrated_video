...
Salva o estado após cada fase bem-sucedida em checkpoint_<nome_oracao>.json.
Permite retomar de qualquer fase sem repetir trabalho anterior.

O arquivo é isolado por oração (nome_oracao) — assim o checkpoint de
"pai_nosso" nunca é confundido com o de "40_Matt_01" ou qualquer outro
projeto rodado no mesmo ambiente/sessão.

Uso:
    cp = Checkpoint(nome_oracao="pai_nosso")
...
ARQUIVO_CHECKPOINT_PADRAO = Path("checkpoint.json")


def _caminho_para_oracao(nome_oracao: str) -> Path:
    """Gera um nome de arquivo de checkpoint seguro e único por oração."""
    slug = re.sub(r"[^a-zA-Z0-9_-]", "_", nome_oracao)
    return Path(f"checkpoint_{slug}.json")


class Checkpoint:
    """Gerencia o estado persistido do pipeline em disco, isolado por oração."""

    def __init__(
        self,
        nome_oracao: Optional[str] = None,
        caminho: Optional[Path] = None,
    ) -> None:
        """
        Args:
            nome_oracao: nome do projeto/oração (ex: "pai_nosso", "40_Matt_01").
                         Gera o arquivo "checkpoint_<nome_oracao>.json".
            caminho:     caminho explícito do arquivo de checkpoint — tem
                         prioridade sobre `nome_oracao` se ambos forem passados.
                         Sem nenhum dos dois, cai no "checkpoint.json" antigo
                         (compatibilidade retroativa) e emite um aviso, já que
                         esse modo compartilha o checkpoint entre orações.
        """
        if caminho is not None:
            self._caminho = Path(caminho)
        elif nome_oracao:
            self._caminho = _caminho_para_oracao(nome_oracao)
        else:
            self._caminho = ARQUIVO_CHECKPOINT_PADRAO
            logger.warning(
                "Checkpoint() sem 'nome_oracao' — usando '%s' compartilhado entre "
                "orações. Passe nome_oracao=config.NOME_ORACAO para isolar por projeto.",
                ARQUIVO_CHECKPOINT_PADRAO,
            )
